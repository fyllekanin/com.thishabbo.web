...
class ForumService {
    ...
    public function postSiteMessageIfApplicable (Thread $thread) {
        if ($thread->category->template !== ConfigHelper::getCategoryTemplatesConfig()->QUEST) {
            return;
        }
        if (!$thread->isApproved) {
            return;
        }
        if (LogSitecp::where('action', Action::getAction(Action::CREATED_SITE_MESSAGE))->where('contentId', $thread->threadId)->count() > 0) {
            return;
        }

        Logger::sitecp(0, 0, Action::CREATED_SITE_MESSAGE, [], $thread->threadId);
    ...
    public function getSlimPost ($postId) {
        ...
        return [
            ...
            'page' => DataHelper::getTotal($post->thread->posts)
        ];
    }
