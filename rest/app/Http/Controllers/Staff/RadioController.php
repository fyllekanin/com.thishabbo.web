...
class RadioController extends Controller {
    ...
    public function getBookingLog ($page) {
        ...
        $bookedPermAction = Action::getAction(Action::BOOKED_PERM_RADIO_SLOT);
        $deletedPermAction = Action::getAction(Action::DELETED_PERM_RADIO_SLOT);
    ...
    public function createRequest (Request $request) {
        ...
        $isRequestingToQuick = RadioRequest::where('ip', $ipAddress)->where('createdAt', '>', $this->nowMinus15)->count() > 0;
    ...
    public function createBooking (Request $request) {
        ...
        $existing = Timetable::radio()->where('day', $booking->day)->where('hour', $booking->hour)->isActive()->count() > 0;
    ...
    public function createPermShow (Request $request) {
        ...
        $isPermExisting = Timetable::where('day', $booking->day)->where('hour', $booking->hour)->isPerm()->count() > 0;
        Condition::precondition($isPermExisting, 400, 'Perm show already exists on this slot');

        $existing = Timetable::where('day', $booking->day)->where('hour', $booking->hour)->isActive()->first();
        if ($existing) {
            $existing->delete();
        }
        ...
        Logger::staff($user->userId, $request->ip(), Action::BOOKED_PERM_RADIO_SLOT, [
            'timetableId' => $timetable->timetableId
        ]);
        return response()->json();
    }
    ...
    public function updatePermShow (Request $request, $timetableId) {
        ...
        $existingPerm = Timetable::where('day', $booking->day)->where('hour', $booking->hour)->isPerm()->first();
        $existingCheck = isset($existingPerm) ? $existingPerm->timetableId != $booking->timetableId : false;
        ...
        Logger::staff($user->userId, $request->ip(), Action::EDITED_PERM_RADIO_SLOT, [
            'show' => $booking->name
        ]);
    ...
    public function deletePermShow (Request $request, $timetableId) {
        ...
        Logger::staff($user->userId, $request->ip(), Action::DELETED_PERM_RADIO_SLOT, [
