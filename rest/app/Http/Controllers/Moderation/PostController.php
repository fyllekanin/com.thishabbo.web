...
class PostController extends Controller {
    ...
    public function mergePosts(Request $request, $threadId) {
        ...
        $postIds = Iterables::filter(
            $postIds,
            function ($postId) use ($firstPostId) {
        ...
        $thread->posts = Post::where('threadId', $thread->threadId)->isApproved()->count();
        $thread->save();

        $firstPost->content = $content;
        $firstPost->save();

        Logger::mod($user->userId, $request->ip(), LogType::MERGE_POSTS, ['postIds' => $request->input('postIds')]);
    ...
    public function approvePosts(Request $request) {
        ...
        foreach ($postIds as $postId) {
            ...
            $post->thread->posts = Post::where('threadId', $post->thread->threadId)->isApproved()->count();
            $post->thread->save();

            $this->myForumService->updateLastPostIdOnCategory($post->thread->categoryId);
            Logger::mod($user->userId, $request->ip(), LogType::APPROVED_POST, ['thread' => $post->thread->title]);
        }
    ...
    public function unApprovePosts(Request $request) {
        ...
        foreach ($postIds as $postId) {
            ...
            if (!$this->myForumService->doCategoryHaveOption($post->thread->categoryId, CategoryOptions::POSTS_DONT_COUNT)) {
                User::where('userId', $post->userId)->update(['posts' => DB::raw('posts - 1')]);
            }

            if ($post->thread->lastPostId == $post->postId) {
                $post->thread->lastPostId = Post::where('isApproved', true)
                    ->where('threadId', $post->thread->threadId)
                    ->orderBy('postId', 'DESC')
                    ->select('postId')
                    ->first()
                    ->postId;
            ...
            $this->myForumService->updateLastPostIdOnCategory($post->thread->categoryId);
            Logger::mod($user->userId, $request->ip(), LogType::UNAPPROVED_POST, ['thread' => $post->thread->title]);
        }
    ...
    public function changeOwner(Request $request) {
        ...
        Condition::precondition(
            !PermissionHelper::haveForumPermission(
                $user->userId,
                CategoryPermissions::CAN_CHANGE_OWNER,
                $posts->get(0)->thread->categoryId
            ),
            400,
            'You do not have permission to change post owner'
        );

        $sameThreadId = Iterables::every(
            $posts,
            function ($post) use ($threadId) {
        ...
        Logger::modMultiple($user->userId, $request->ip(), LogType::CHANGE_POST_OWNER, $logData);
    ...
    private function deletePost($postId, $user, $ipAddress) {
        ...
        $this->myForumService->updateLastPostIdOnCategory($post->thread->categoryId);
        Logger::mod($user->userId, $ipAddress, LogType::DELETED_POST, ['thread' => $post->thread->title]);
    }
}
