    ...
    var defaultCommandsOverrides = {
        ...
        quotepost: {
            ...
            txtExec: function (caller, selected) {
                ...
                getEditorCommand('quotepost')._dropDown(
                    editor,
                    caller,
                    function (postId, text) {
                        editor.insertText('[quotepost=' + postId + ']' + text + '[/quotepost]');
                    }
                );
            }
        },
        spoiler: {
            ...
            txtExec: function (caller) {
                ...
                getEditorCommand('spoiler')._dropDown(
                    editor,
                    caller,
                    function (userId, text) {
                        editor.insertText('[spoiler]' + text + '[/spoiler]');
                    }
                );
            }
        },
        mention: {
            ...
            txtExec: function (caller) {
                ...
                getEditorCommand('mention')._dropDown(
                    editor,
                    caller,
                    function (userId, text) {
                        editor.insertText('[mention]' + text + '[/mention]');
                    }
                );
            }
        },
        taggroup: {
            ...
            txtExec: function (caller) {
                ...
                getEditorCommand('taggroup')._dropDown(
                    editor,
                    caller,
                    function (groupId, text) {
                        editor.insertText('[taggroup]' + text + '[/taggroup]');
                    }
                );
            }
        },
    ...
    var bbcodeHandlers = {
        ...
        imgr: {
            ...
            format: function (element, content) {
                ...
                return '[imgr' + attribs + ']' + attr(element, 'src') + '[/imgr]';
            },
            html: function (token, attrs, content) {
                var undef, width, height, match,
                    attribs = '';
                ...
                return '<img' + attribs +
                    ' src="' + escapeUriScheme(content) + '" align="right"/>';
            }
        },
        // END_COMMAND

        // START_COMMAND: Image Left
        imgl: {
            allowsEmpty: true,
            tags: {
                img: {
            ...
            format: function (element, content) {
                ...
                return '[imgl' + attribs + ']' + attr(element, 'src') + '[/imgl]';
            },
            html: function (token, attrs, content) {
                var undef, width, height, match,
                    attribs = '';
                ...
                return '<img' + attribs +
                    ' src="' + escapeUriScheme(content) + '" align="left"/>';
            }
        },
        // END_COMMAND
        ...
        quotepost: {
            ...
            format: function (element, content) {
                var postId = attr(element, 'data-post-id');
                return '[quotepost=' + postId + ']' + content + '[/quotepost]';
            }
        },
        // END_COMMAND
        ...
        spoiler: {
            ...
            format: function (element, content) {
                var text = element.innerHTML.replace('<summary>Spoiler</summary>', '');
                return '[spoiler]' + text + '[/spoiler]';
            }
        },

        // START_COMMAND: mention
        mention: {
            tags: {
                a: {
                    'data-user': null
            ...
            quoteType: QuoteType.never,
            html: function (token, attrs, content) {
                return '<a class="mention-user" data-user="' + content + '">' + content + '</a>';
            },
            format: function (element, content) {
                return '[mention]' + content + '[/mention]';
            }
        },
        // START_COMMAND: mentionGroup
        taggroup: {
            tags: {
                a: {
                    'data-tag-group': null
            ...
            quoteType: QuoteType.never,
            html: function (token, attrs, content) {
                return '<a class="mention-group" data-tag-group="' + content + '">' + content + '</a>';
            },
            format: function (element, content) {
