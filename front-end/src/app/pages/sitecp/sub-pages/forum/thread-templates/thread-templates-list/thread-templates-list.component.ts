...
import { ThreadTemplate } from '../thread-template.model';


@Component({
    selector: 'app-sitecp-forum-thread-templates-list',
    templateUrl: 'thread-templates-list.component.html'
})
export class ThreadTemplatesListComponent extends Page implements OnDestroy {
    private _threadTemplates: Array<ThreadTemplate> = [];
    ...
    onAction (action: Action): void {
        const prefix = this._threadTemplates.find(item => item.prefixId === Number(action.rowId));
        switch (action.value) {
    ...
    private onDelete (prefix: Prefix): void {
        this._httpService.delete(`sitecp/prefixes/${prefix.prefixId}`)
            .subscribe(() => {
                this._threadTemplates = this._threadTemplates.filter(pre => pre.prefixId !== prefix.prefixId);
    ...
    private onData (data: { data: Array<Prefix> }): void {
        this._threadTemplates = data.data;
        this.createOrUpdateTable();
    }
    ...
    private getTableRows (): Array<TableRow> {
        ...
        return this._threadTemplates.map(prefix => {
            return new TableRow({
