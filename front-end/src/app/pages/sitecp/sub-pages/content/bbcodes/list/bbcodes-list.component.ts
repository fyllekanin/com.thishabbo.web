...
import { MANAGE_BBCODES_BREADCRUMB_ITEM, SITECP_BREADCRUMB_ITEM } from '../../../../sitecp.constants';


@Component({
    selector: 'app-sitecp-content-bbcode-list',
...
export class BBcodesListComponent extends Page implements OnDestroy {
    ...
    onAction (action: Action): void {
        ...
        switch (action.value) {
            case BBcodeActions.EDIT_BBCODE:
                this._router.navigateByUrl(`/sitecp/content/bbcodes/${action.rowId}`);
                break;
    ...
    private onDelete (bbcode: BBcodeModel): void {
        this._httpService.delete(`sitecp/content/bbcodes/${bbcode.bbcodeId}`)
            .subscribe(() => {
        ...
        return list.map(bbcode => {
            return new TableRow({
                ...
                cells: [
                    ...
                    new TableCell({
                        title: `<img src="/resources/images/emojis/${bbcode.bbcodeId}.gif" />`,
                        innerHTML: true
                    })
                ],
                actions: actions
