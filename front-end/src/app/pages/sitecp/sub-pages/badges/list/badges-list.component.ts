...
export class BadgesListComponent extends Page implements OnDestroy {
    ...
    onAction (action: Action): void {
        switch (action.value) {
            case BadgeListActions.EDIT_BADGE:
                this._router.navigateByUrl(`/sitecp/badges/${action.rowId}`);
                break;
            ...
            case BadgeListActions.MANAGE_USERS:
                this._router.navigateByUrl(`/sitecp/badges/${action.rowId}/users`);
                break;
        }
    }
    ...
    private onDelete (badgeId: number): void {
        this._httpService.delete(`sitecp/badges/${badgeId}`)
            .subscribe(() => {
