...
export class SubscriptionsService implements Resolve<UserSubscriptionsPage> {
    ...
    resolve (route: ActivatedRouteSnapshot): Observable<UserSubscriptionsPage> {
        ...
        return this._httpService.get(`admin/users/${userId}/subscriptions`)
    ...
    create (userId: number, userSubscription: UserSubscriptionPayload): Promise<UserSubscriptionItem> {
        return new Promise(res => {
            this._httpService.post(`admin/users/${userId}/subscriptions`, {data: userSubscription})
                .subscribe(response => {
    ...
    update (userSubscription: UserSubscriptionPayload): void {
        this._httpService.put(`admin/users/subscriptions/${userSubscription.userSubscriptionId}`, {data: userSubscription})
            .subscribe(() => {
                this._notificationService.sendInfoNotification('User subscription updated');
            }, this._notificationService.failureNotification.bind(this._notificationService));
    }
    ...
    delete (userSubscriptionId: number): Promise<void> {
        return new Promise(res => {
            this._dialogService.openConfirmDialog(
                'Are you sure?',
                'Are you sure you wanna dele this user subscription?',
                () => {
                    this._httpService.delete(`admin/users/subscriptions/${userSubscriptionId}`)
                        .subscribe(() => {
